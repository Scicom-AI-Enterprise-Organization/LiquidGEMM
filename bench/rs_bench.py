...
for name, N, K in LLAMA:
    ...
    spk = ops.build_rs_scale_pack(qw).cuda()
    ...
    ok = torch.equal(
        torch.ops.liquidgemm.w4a8_wgmma_rs(xc, rs, spk, su8, off, N, K, 64, True),
    ...
    for M in [64, 128, 256, 512, 1024]:
        ...
        tb = t(lambda: Xg @ Wg.t())
        tr = t(lambda: torch.ops.liquidgemm.w4a8_wgmma_rs(xi, rs, spk, su8, off, N, K, 64, True))
